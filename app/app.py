...
class Form:
    def __init__(self):
        ...
        self.sunshine = st.number_input('⛈️ The number of hours of bright sunshine in the day', min_value=0,
                                        max_value=24,
                                        value=0,
                                        step=1)
        ...
        self.windSpeed3pm = st.number_input('🌬 Wind speed (km/hr) averaged over 10 minutes prior to 3pm', min_value=0.0,
                                            value=0.0, step=0.1)
        self.humidity9am = st.number_input('💦 Humidity (percent) at 9am', min_value=0.0, value=0.0, max_value=100.0,
                                           step=0.1)
        self.humidity3pm = st.number_input('💦 Humidity (percent) at 3pm', min_value=0.0, value=0.0, max_value=100.0,
                                           step=0.1)
        ...
        self.pressure3pm = st.number_input('💭 Atmospheric pressure (hpa) reduced to mean sea level at 3pm',
                                           min_value=0.0,
                                           value=0.0, step=0.1)
        self.cloud9am = st.number_input('🌫 Fraction of sky (oktas) obscured by cloud at 9am', min_value=0.0,
                                        max_value=8.0,
                                        value=0.0, step=0.1)
        self.cloud3pm = st.number_input('🌫 Fraction of sky (oktas) obscured by cloud at 3pm', min_value=0.0,
                                        max_value=8.0,
                                        value=0.0, step=0.1)
